...
const TableMataKuliah = memo(() => {
  ...
  return (
    <Card>
      <HeaderPage title='Manajemen Matakuliah' />
...
const MatakuliahEntries = memo(({ search, columns }: { search: string; columns: GridColDef[] }) => {
  const dispatch = useAppDispatch()
  const { isRefresh } = useAppSelector(state => state.matakuliah)
  ...
  const handleGetData = async (isPagination = false) => {
    ...
    // @ts-ignore
    await dispatch(getAllMatakuliah({ data: body }))
      .then((res: any) => {
        if (
          !(res.payload.content?.entries ?? []).some((obj: any) =>
            (data?.entries ?? []).some((newObj: any) => obj.kode === newObj.kode)
          ) &&
          isPagination
        ) {
