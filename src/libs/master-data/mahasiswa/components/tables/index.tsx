...
import { getAllMahasiswa } from 'src/stores/master-data/mahasiswa/action'
...
const TableMahasiswa = memo(() => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <MahasiswaEntries search={search} columns={columns} />
      </CardContent>
    </Card>
  )
})
...
const MahasiswaEntries = memo(({ search, columns }: { search: string; columns: GridColDef[] }) => {
  const dispatch = useAppDispatch()
  const { isRefresh } = useAppSelector(state => state.mahasiswa)
  ...
  const handleGetData = async (isPagination = false) => {
    ...
    // @ts-ignore
    await dispatch(getAllMahasiswa({ data: body }))
      .then((res: any) => {
        if (
